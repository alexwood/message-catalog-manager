...
var buildData = function (data, addInsertsToData, namedInserts, positionalInserts) {
    var outputData = {};

    if(addInsertsToData){
        outputData = Object.assign(namedInserts);
    }
...
MessageCatalogManager.prototype.getMessage = function (catalogName, code, namedInserts, positionalInserts, language, verbose) {
    ...
    var addInsertsToData = false;
    if (message.addInsertsToData) {
        addInsertsToData = message.addInsertsToData;
        message.addInsertsToData = undefined;
    }

    if (message.data) {
        //apply Inserts to detail
        message.data = buildData(message.data, addInsertsToData, namedInserts, positionalInserts);
    }
